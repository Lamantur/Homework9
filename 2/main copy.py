class Singleton(type):
    ...
    def __call__(self, *args, **kwds):
        if self.value is None:
            self.value = super(Singleton, self).__call__(*args, **kwds)
        return self.value
...
ivanivanov = Position("Вася", "Иванов", 40000, "Заведующий хозяйством")
petrov = Position("Петя", "Петров", 30000, "Менеджер")

print(ivanivanov.get_total_income(ivanivanov.income))
print(ivanivanov)
print(ivanivanov is petrov)
